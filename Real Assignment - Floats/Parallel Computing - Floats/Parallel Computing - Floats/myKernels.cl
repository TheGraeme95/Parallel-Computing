...
__kernel void sort(__global const float* input, __local float* localMem, __global float* output, int merge)
{
    int global_ID = get_global_id(0);
    int local_ID = get_local_id(0);
    int group_size = get_group_id(0);
    int N = get_local_size(0);
	
    int offset = global_ID + ((N/2) * merge); // offset the beginning of the array
    ...
    for (int l = 1; l < N; l <<= 1)
    {
        bool direction = ((local_ID & (l<<1)) != 0);

        for (int inc = l; inc > 0; inc >>= 1)	//Bitonic merge
        {
            int j = local_ID ^ inc;
            float i_data = localMem[local_ID]; // define two different directions effectively being the start and end
            float j_data = localMem[j];			// of the vector.
            ...
            localMem[local_ID] = (swap) ? j_data : i_data; // Swap the data according to whether
            barrier(CLK_LOCAL_MEM_FENCE);
        }
    }
